...
{
    private static final Logger log = Logger.get(LokiRecordCursor.class);

    private final List<LokiColumnHandle> columnHandles;
    private final int[] fieldToColumnIndex;

    private final Iterator<LokiStandardizedRow> metricsItr;
    private final long totalBytes;

    private LokiStandardizedRow fields;

    public LokiRecordCursor(List<LokiColumnHandle> columnHandles, ByteSource byteSource)
    {
        ...
        try (CountingInputStream input = new CountingInputStream(byteSource.openStream())) {
            metricsItr = lokiResultsInStandardizedForm(new LokiQueryResponseParse(input).getResults()).iterator();
...
//        checkFieldType(field, createUnboundedVarcharType());
    ...
    private List<LokiStandardizedRow> lokiResultsInStandardizedForm(List<LokiMetricResult> results)
    {
        log.info(results.size() + " result size");
        return results.stream().map(result ->
                result.getTimeSeriesValues().getValues().stream().map(lokiTimeSeriesValue ->
                        new LokiStandardizedRow(
                                new JSONObject(Map.copyOf(result.getMetricHeader())).toJSONString(),
//                                getBlockFromMap(columnHandles.get(0).getColumnType(), metricHeaderToMap(result.getMetricHeader())),
                                lokiTimeSeriesValue.getTimestamp(),
                                lokiTimeSeriesValue.getValue()))
